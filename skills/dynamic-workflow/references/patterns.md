...
Use for read-only classification or summarization. Enumerate items, call one agent per item, store structured results, and aggregate a report.

## Map Review Write

Use when edits need verification. An editor agent proposes a full replacement, patch, or structured edit. A reviewer agent checks the proposal. Deterministic workflow code writes accepted changes and records rejected changes.
